...
<!-- general -->
	<div class="general">
		<h4 class="latest-text w3_latest_text"><?php echo $_GET["isim"]; ?> Araması Sonuçları</h4>
		<div class="container">
			<div class="bs-example bs-example-tabs" role="tabpanel" data-example-id="togglable-tabs">
              ...
              $aranan= $_GET["isim"];
							...
							$arama= $dizi->arama($aranan);
              if(count($arama[0]) > 0){
							for ($i=0; $i <count($arama[0]); $i++) {
								$dizi= $arama[0][$i];
            ...
            echo "<h1>Aranan içerik bulunamadı</h1>";
          }
						?>
						...
						<nav aria-label="Page navigation ">
						  <ul class="pagination justify-content-end">
						    <?php
								...
								for ($i=1; $i <= $arama[1]["max"] ; $i++) {
									if( ($sayfa==$i) || ($i==$arama[1]["bulunan"])){
